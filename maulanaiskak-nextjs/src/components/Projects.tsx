...
const PROJECT_SCENES: { variant: SceneVariant; color: string }[] = [
  { variant: 'torusknot',        color: '#00d4ff' },
  { variant: 'wireSphere',       color: '#7c3aed' },
  { variant: 'octahedron',       color: '#00d4ff' },
  { variant: 'bars',             color: '#10b981' },
  { variant: 'twinBoxes',        color: '#f59e0b' },
  { variant: 'icosahedron',      color: '#ef4444' },
  { variant: 'organicParticles', color: '#10b981' },
];
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#00d4ff]/30 h-full"
        style={{ transition: 'transform 0.15s ease, border-color 0.3s' }}
      >
        <div className="relative h-44 bg-[#0a0a0a]">
          {inView && (
            <MiniScene variant={scene.variant} color={scene.color} className="absolute inset-0" isHovered={isHovered} />
          )}
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {project.technologies.slice(0, 4).map((tech) => (
              <span key={tech} className="text-xs px-2 py-1 rounded-full bg-[#00d4ff]/10 text-[#00d4ff] border border-[#00d4ff]/20">
                {tech}
              </span>
            ))}
